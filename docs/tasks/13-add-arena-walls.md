...
## 1. Add the wall container

1. Open ==scenes/Main.tscn==.

2. In the Scene panel, ==right-click== Main and choose **Add Child Node**. Search for ==StaticBody2D== and click **Create**.

3. Rename it ==ArenaBounds==.

## 2. Make the top wall

1. Add a ==CollisionShape2D== as a child of ==ArenaBounds== and rename it ==TopWall==.

2. Set its **Shape** to **New RectangleShape2D**. Open the new shape and set **Size** to x ==1280==, y ==16==.

3. In **Transform**, set **Position** to x ==640==, y ==8==.

## 3. Add the other walls

1. ==Right-click== ==ArenaBounds== and add a new ==CollisionShape2D==. Rename it ==BottomWall==,
   give it a **New RectangleShape2D**, and use the size and position below.

2. Repeat that step to create ==LeftWall== and ==RightWall==. Give every wall its own new
   ==RectangleShape2D==.
...
## 4. Let the walls stop the player

1. Open ==scenes/Player.tscn==, select the ==Player== root node, and click its script icon.

2. Replace the whole script with the code below. This is the complete player script
